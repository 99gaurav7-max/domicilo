...
import { TableContainer, Card, StatusBadge, EmptyState, SearchInput, Select } from '../../components/ui/Table';
      ...
      >
        {loading ? (
          ...
          <EmptyState icon={<Home className="w-8 h-8" />} title="No tenants yet"
            ...
            action={<button onClick={() => setShowCreate(true)} className="btn-primary text-sm">Add Tenant</button>} />
        ) : (
          <>
            <thead>
        ...
        {showFine && (
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50">
              <p className="text-sm text-gray-600 dark:text-gray-400">Tenant: <span className="font-medium text-gray-900 dark:text-white">{showFine.full_name}</span></p>
              <p className="text-sm text-gray-600 dark:text-gray-400">Monthly Rent: <span className="font-medium">₹{(showFine.rent_amount || showFine.room_rent || 0).toLocaleString()}</span></p>
            </div>
